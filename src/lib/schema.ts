...
export function publisherLocalBusiness(city: City) {
  return {
    ...
    description: `${site.name} is a directory that publishes city pages for roofing and routes quote requests only to approved listed companies. ${site.name} is not a roofing contractor and does not perform field work.`,
    url: site.url,
    email: site.email,
    areaServed: {
      "@type": "City",
